/**
 * SQLite database access layer for the India BIS IT Standards MCP server.
 *
 * Schema:
 *   - frameworks  — BIS standard categories and adopted ISO frameworks
 *   - controls    — Individual IS standard entries and technical rules
 *   - circulars   — IT Act rules and BIS notifications
 *
 * FTS5 virtual tables back full-text search on controls and circulars.
 *
 * NOTE: Only catalog metadata is publicly accessible from BIS.
 * Full standards text requires a BIS subscription at services.bis.gov.in.
 */
